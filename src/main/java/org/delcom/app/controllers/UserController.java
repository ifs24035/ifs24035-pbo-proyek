...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<Map<String, UUID>>> registerUser(@RequestBody User reqUser) {
        ...
        if (existingUser != null) {
            return ResponseEntity.badRequest().body(new ApiResponse<>("fail", "Pengguna sudah terdaftar dengan email ini", null));
        }

        String hashPassword = new BCryptPasswordEncoder().encode(reqUser.getPassword());
        User createdUser = userService.createUser(reqUser.getName(), reqUser.getEmail(), hashPassword);

        return ResponseEntity.ok().body(new ApiResponse<>("success", "Berhasil melakukan pendaftaran", Map.of("id", createdUser.getId())));
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, String>>> loginUser(@RequestBody User reqUser) {
        ...
        boolean isPasswordMatch = new BCryptPasswordEncoder().matches(reqUser.getPassword(), existingUser.getPassword());
        if (!isPasswordMatch) {
            return ResponseEntity.badRequest().body(new ApiResponse<>("fail", "Email atau password salah", null));
        }
        ...
        return ResponseEntity.ok().body(new ApiResponse<>("success", "Login berhasil", Map.of("authToken", jwtToken)));
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, User>>> getUserInfo() {
        ...
        return ResponseEntity.ok(new ApiResponse<>("success", "Berhasil mendapatkan info user", Map.of("user", authUser)));
    }
    ...
    public ResponseEntity<ApiResponse<User>> updateUser(@RequestBody User reqUser) {
        ...
        User updatedUser = userService.updateUser(authUser.getId(), reqUser.getName(), reqUser.getEmail());
        if (updatedUser == null) {
            return ResponseEntity.status(404).body(new ApiResponse<>("fail", "User tidak ditemukan", null));
        }

        return ResponseEntity.ok(new ApiResponse<>("success", "User berhasil diupdate", null));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> updateUserPassword(@RequestBody Map<String, String> passwordPayload) {
        if (!authContext.isAuthenticated()) {
            return ResponseEntity.status(401).body(new ApiResponse<>("fail", "Autentikasi tidak valid", null));
        }
        ...
        boolean isPasswordMatch = new BCryptPasswordEncoder().matches(oldPassword, authUser.getPassword());
        if (!isPasswordMatch) {
